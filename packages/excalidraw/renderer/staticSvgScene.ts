...
import { getElementAbsoluteCoords, getCommonBounds } from "../element";
import {
...
// import { getFreeDrawSvgPath, IMAGE_INVERT_FILTER, exportableUnconditionalRenderElement, } from "./renderElement";
import { getFreeDrawSvgPath, IMAGE_INVERT_FILTER, } from "./renderElement";

// const myapdertertpstate = AppState.zoom.value
const roughSVGDrawWithPrecision = (rsvg: RoughSVG, drawable: Drawable, precision?: number,) => {
  if (typeof precision === "undefined") {
    return rsvg.draw(drawable);
  }
  const pshape: Drawable = {sets: drawable.sets, shape: drawable.shape, options: { ...drawable.options, fixedDecimalPlaceDigits: precision },};
  return rsvg.draw(pshape);
};

const maybeWrapNodesInFrameClipPath = (element: NonDeletedExcalidrawElement, root: SVGElement, nodes: SVGElement[], frameRendering: AppState["frameRendering"], elementsMap: RenderableElementsMap,) => {
...
const renderElementToSvg = (element: NonDeletedExcalidrawElement, elementsMap: RenderableElementsMap, rsvg: RoughSVG, svgRoot: SVGElement, files: BinaryFiles, offsetX: number, offsetY: number, renderConfig: SVGRenderConfig, svgwidth: number, svgheight: number) => {
  
  
  const degree = (180 * element.angle) / Math.PI;
  const opacity = ((getContainingFrame(element, elementsMap)?.opacity ?? 100) * element.opacity) / 10000;
  ...
  if (myelement.isMakeClippingMask && myelement.clippingMaskId && myelement.cmGroupLength && myelement.cmeIndex) {
    ...
    if (maskedElementsMap.get(myelement.clippingMaskId).length && maskedElementsMap.get(myelement.clippingMaskId).length === myelement.cmGroupLength) {
      ...
      node.setAttribute("transform", `translate(${offsetX || 0} ${offsetY || 0})`);
      ...
      image.setAttribute("href", document.getElementById(myelement.clippingMaskId)?.getAttribute("src") || "");
    ...
    if (isTextElement(element)) {
      ...
      if (isArrowElement(container)) {
        ...
        const boundTextCoords = LinearElementEditor.getBoundTextElementPosition(container, element as ExcalidrawTextElementWithContainer, elementsMap,);
    ...
    switch (element.type) {
      ...
      case "rectangle": {
        if (element.isPathElement) {
          const node = svgRoot.ownerDocument.createElementNS(SVG_NS, "g");
          var sFactorX = element.width/element.constWidth, sFactorY = element.height/element.constHeight;
          ...
          path.setAttribute("transform", `translate(${(-element.constx*sFactorX)} ${(-element.consty*sFactorY)})`);
          path.setAttribute("fill", element.backgroundColor);
          path.setAttribute("stroke", element.strokeColor);
          path.setAttribute("d", LinearElementEditor.generatesvgPropToPath(element.pathObjects,sFactorX,sFactorY));
      ...
      case "embeddable": {
        ...
        renderElementToSvg(label, elementsMap, rsvg, root, files, label.x + offset.x - element.x, label.y + offset.y - element.y, renderConfig, svgwidth, svgheight,);
      ...
      case "freedraw": {
        ...
        const node = backgroundFillShape ? roughSVGDrawWithPrecision(rsvg, backgroundFillShape, MAX_DECIMALS_FOR_SVG_EXPORT,) : svgRoot.ownerDocument!.createElementNS(SVG_NS, "g");
        console.log(node);
          ...
          if (element && 'id' in element && 'width' in element && 'height' in element) {
            if (element.roundness) {
              if ('isCroppedImage' in element && 'cropProperties' in element && element.isCroppedImage) {
                ...
                const radius = getCornerRadius(Math.min(cropProperties.width, cropProperties.height), element,);
                clipRect.setAttribute("width", `${cropProperties.width}`);
                clipRect.setAttribute("height", `${cropProperties.height}`);
                clipRect.setAttribute("x", `${cropProperties.x}`);
                clipRect.setAttribute("y", `${cropProperties.y}`);
...
export const renderSceneToSvg = (elements: readonly NonDeletedExcalidrawElement[], elementsMap: RenderableElementsMap, rsvg: RoughSVG, svgRoot: SVGElement, files: BinaryFiles, renderConfig: SVGRenderConfig, svgwidth: number, svgheight: number,) => {
  if (!svgRoot) {
    return;
  }
